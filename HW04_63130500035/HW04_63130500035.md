...
ใช้ `group_by`,`select()`,`summarise()` จาก `dplyr` Package          <br>
โดย `group_by` --> นำ data frame และ variables <b>Type</b> มาจัดกลุ่ม
`select()` --> เลือกใช้ variable ที่ชื่อว่า <b>Price</b>
`summarise()` --> ทำการสรุปโดยหาค่าเฉลี่ยของ <b>Price</b>
