...
ตัวอย่างนี้จะใช้ `forcats` โดยใช้ชุดข้อมูลก่อนหน้านี้(implement to dataset 'comsci')

```
comsci %>%
  mutate(Type = fct_infreq(Type)) %>%
  ggplot(aes(x = Type)) +    
  geom_bar(aes(color=Type)) +            # classified by the specified xxx
...
ตัวอย่างนี้จะใช้ `stringr` โดยใช้ข้อมูลที่สมมุติขึ้น(not implement to dataset 'comsci')
```
