...
ใช้ `group_by`,`select()`และ`summarise()` จาก `dplyr` Package          <br>
โดย `group_by` --> นำ data frame และ variables <b>Type</b> มาจัดกลุ่ม <br>
`select()` --> เลือกใช้ variable ที่ชื่อว่า <b>Price</b>                   <br>
`summarise()` --> ทำการสรุปโดยหาค่าเฉลี่ยของ <b>Price</b>               <br>
